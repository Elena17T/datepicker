...
const renderCalendar = () => {
  ...
  const lastDay = new Date(
    ...
    date.getMonth() + 1,
    0
  ).getDate(); //the last day of current month

  const prevLastDay = new Date(
    date.getFullYear(),
    date.getMonth(),
    0
  ).getDate(); //the last day of the previous month
  ...
  const lastDayIndex = new Date(
    ...
    date.getMonth(),
    0
  ).getDay(); // return the index number of the last day from previous month

  const nextDays = 7 - lastDayIndex - 1; //count from what day starts week in the next month
  ("");
  ...
  document.querySelector(".date h1").innerHTML =
    months[date.getMonth()] + " " + year; //show current month and year
  let days = "";
  ...
  //created days for the next month
